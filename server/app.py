...
NON_CAMPAIGN_FORMAT_PROMPT = (
    ...
    '"allow_custom": <true|false>}]}. Ask at most 4 questions, each with 2-6 concrete, relevant checkbox options — '
    'the user may select more than one per question (check all that apply), and always include a format question '
    '(options: "Text-only post", "Single image", "Carousel", "Short video / Reel") whenever format isn\'t already '
    'clear. Set "allow_custom" true only for questions where a specific fact (a rate, a name, a date, a promo '
    'detail) might be needed and no fixed option could cover it.\n\n'
...
CAMPAIGN_CLARIFICATION_PROMPT = (
    ...
    'ground the content — recent client wins, testimonials, case studies, or a specific current rate/promo — or '
    'whether placeholders should be used instead, and (c) whether there is a topic, platform priority, or '
    'timeframe already in mind beyond the platform defaults.\n\n'
    ...
    'may select more than one per question. Set "allow_custom" true for questions like real testimonials or '
    'current rates where a fixed option can\'t capture the real answer.'
)
